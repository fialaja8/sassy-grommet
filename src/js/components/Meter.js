...
const _normalizeSeries = (props, thresholds) => {
  let series = [];
  if (props.series) {
    series = props.series;
...
const _normalizeThresholds = (props, min, max) => {
  let thresholds = [];
  if (props.thresholds) {
...
const _stateFromProps = (props) => {
  let total;
  if (props.series) {
    total = _seriesTotal(props.series);
  ...
  if (props.series && props.series.length > 1) {
    seriesMax = _seriesMax(props.series);
  }
  ...
  const thresholds = _normalizeThresholds(props, min, max);
  // Normalize simple value prop to a series, if needed.
  const series = _normalizeSeries(props, thresholds);
